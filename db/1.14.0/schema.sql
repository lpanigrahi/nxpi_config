...
\restrict LmLyE5ZsMqXxJWy3llM6JCTcvSIzAnady661VVpZzeCZ8Mg1G9KFK7S040whegk
...
--
-- NOTE: `_fkey`, not drizzle's `_organization_id_organization_id_fk`. Source
-- migration 0078 declares these inline (`REFERENCES …`), so POSTGRES names
-- them, and every VM that takes migrate-1.11.0.sql gets these names. A fresh
-- install must land the same catalog, so the packaged schema mirrors the DELTA
-- lineage rather than the drizzle-push naming the rest of this dump carries.
-- Diverging here would be invisible until a future delta named a constraint.
--
...
\unrestrict LmLyE5ZsMqXxJWy3llM6JCTcvSIzAnady661VVpZzeCZ8Mg1G9KFK7S040whegk


--
-- HAND-PATCHED ADDITIONS (2026-08-11) — objects the drizzle-derived export
-- baseline cannot express or did not know about. Kept at the end, after the
-- dump's \unrestrict, so regenerated dumps diff cleanly above this marker.
--

-- skill_undeployed_idx: created by app migration 0070 (raw SQL only — absent
-- from schema.pg.ts until 2026-08-11, so drizzle-kit push never realized it
-- and the machine diff for the original 1.9.0 delta DROPPED it). Restored so
-- fresh installs match delta-migrated databases.
CREATE INDEX IF NOT EXISTS skill_undeployed_idx ON public.skill USING btree (user_id) WHERE (deployed = false);
...
-- packaged schema before this carried the control; delta-migrated databases
