...
-- 1.7.0 (migration 0069): the replay envelope store. Listed here as well as in
-- the delta so a FRESH install from schema.sql + grants.sql lands with the same
-- privileges as an upgraded one — the two paths diverging is the neo_gen grant
-- gotcha that has bitten every previous version.
