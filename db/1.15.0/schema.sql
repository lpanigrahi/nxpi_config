...
\restrict Uu3m5rL41VgHitXbh0cfXyNepzrPH6jlKIYbFespjxsrCRmtAraGEkkPrgfAn9M
...
CREATE TABLE public.assistant (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT assistant_visibility_check CHECK (((visibility)::text = ANY ((ARRAY['private'::character varying, 'team'::character varying, 'org'::character varying, 'official'::character varying])::text[])))
);


--
-- Name: assistant_team; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.assistant_team (
...
CREATE TABLE public.audit_chain_head (
    id integer NOT NULL,
    head_signature text DEFAULT ''::text NOT NULL,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL
);
...
CREATE TABLE public.marketplace_listing (
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT marketplace_listing_resource_type_check CHECK (((resource_type)::text = ANY ((ARRAY['agent'::character varying, 'skill'::character varying, 'workflow'::character varying, 'assistant'::character varying, 'prompt'::character varying, 'knowledge_pack'::character varying])::text[]))),
...
CREATE TABLE public.nav_visibility_override (
    ...
    CONSTRAINT nav_visibility_override_scope_org_check CHECK ((((scope)::text = 'global'::text) = (organization_id IS NULL)))
);
...
--
-- Name: org_policy_version; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.org_policy_version (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid NOT NULL,
    ...
    changed_by uuid,
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT org_policy_version_action_check CHECK (((action)::text = ANY ((ARRAY['create'::character varying, 'update'::character varying, 'enable'::character varying, 'disable'::character varying, 'delete'::character varying])::text[])))
);
...
--
-- Name: organization_entitlement; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: plugin_bundle; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.plugin_bundle (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    slug text NOT NULL,
    name text NOT NULL,
    description text,
    publisher text NOT NULL,
    publisher_kind character varying DEFAULT 'platform'::character varying NOT NULL,
    icon json,
    tags text[],
    scope character varying DEFAULT 'enterprise'::character varying NOT NULL,
    team_id uuid,
    organization_id uuid,
    status character varying DEFAULT 'pending_review'::character varying NOT NULL,
    featured boolean DEFAULT false NOT NULL,
    ...
    source_id uuid,
    origin jsonb,
    CONSTRAINT plugin_bundle_publisher_kind_check CHECK (((publisher_kind)::text = ANY ((ARRAY['platform'::character varying, 'partner'::character varying, 'community'::character varying])::text[]))),
    CONSTRAINT plugin_bundle_scope_check CHECK (((scope)::text = ANY ((ARRAY['personal'::character varying, 'team'::character varying, 'organization'::character varying, 'enterprise'::character varying, 'public'::character varying])::text[]))),
    CONSTRAINT plugin_bundle_status_check CHECK (((status)::text = ANY ((ARRAY['draft'::character varying, 'pending_review'::character varying, 'approved'::character varying, 'published'::character varying, 'rejected'::character varying, 'suspended'::character varying])::text[])))
);


--
-- Name: plugin_bundle_install; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.plugin_bundle_install (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
--
-- Name: plugin_bundle_item; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.plugin_bundle_item (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    bundle_id uuid NOT NULL,
    resource_type character varying NOT NULL,
    resource_id uuid NOT NULL,
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT plugin_bundle_item_resource_type_check CHECK (((resource_type)::text = ANY ((ARRAY['agent'::character varying, 'skill'::character varying, 'workflow'::character varying, 'assistant'::character varying, 'prompt'::character varying, 'knowledge_pack'::character varying, 'mcp'::character varying])::text[])))
);


--
-- Name: plugin_source; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.plugin_source (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    name text NOT NULL,
    slug text NOT NULL,
    ...
    updated_at timestamp without time zone DEFAULT now() NOT NULL,
    deleted_at timestamp without time zone,
    CONSTRAINT plugin_source_last_sync_status_check CHECK (((last_sync_status IS NULL) OR ((last_sync_status)::text = ANY ((ARRAY['ok'::character varying, 'error'::character varying])::text[])))),
    CONSTRAINT plugin_source_source_type_check CHECK (((source_type)::text = ANY ((ARRAY['github'::character varying, 'git_url'::character varying, 'url'::character varying])::text[]))),
    CONSTRAINT plugin_source_status_check CHECK (((status)::text = ANY ((ARRAY['active'::character varying, 'disabled'::character varying])::text[])))
);


--
-- Name: plugin_source_entry; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.plugin_source_entry (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
--
-- Name: semantic_response_cache; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.semantic_response_cache (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    organization_id uuid,
...
CREATE TABLE public.skill (
    ...
    last_auto_rollback_at timestamp without time zone,
    lifecycle_status character varying DEFAULT 'approved'::character varying NOT NULL,
    lifecycle_reviewed_by uuid,
    lifecycle_reviewed_at timestamp without time zone,
    lifecycle_note text,
    lifecycle_previous_status character varying,
    qa_baseline_hash text,
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    updated_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    deleted_at timestamp without time zone,
    CONSTRAINT skill_lifecycle_status_check CHECK (((lifecycle_status)::text = ANY ((ARRAY['draft'::character varying, 'pending'::character varying, 'approved'::character varying, 'rejected'::character varying, 'disabled'::character varying, 'deprecated'::character varying, 'retired'::character varying, 'archived'::character varying])::text[]))),
    CONSTRAINT skill_scope_check CHECK (((scope)::text = ANY ((ARRAY['personal'::character varying, 'team'::character varying, 'organization'::character varying])::text[]))),
    CONSTRAINT skill_visibility_check CHECK (((visibility)::text = ANY ((ARRAY['public'::character varying, 'private'::character varying, 'readonly'::character varying])::text[])))
);


--
-- Name: skill_attestation; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.skill_attestation (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
...
--
-- Name: skill_install; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.skill_install (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    skill_id uuid NOT NULL,
    user_id uuid NOT NULL,
    installed_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
...
--
-- Name: skill_qa_certification; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.skill_qa_certification (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    skill_id uuid NOT NULL,
    run_id uuid,
    ...
    revoked_by uuid,
    revoked_reason text,
    CONSTRAINT skill_qa_certification_tier_check CHECK (((tier)::text = ANY ((ARRAY['none'::character varying, 'bronze'::character varying, 'silver'::character varying, 'gold'::character varying, 'enterprise'::character varying])::text[])))
);


--
-- Name: skill_qa_check_result; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.skill_qa_check_result (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    ...
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT skill_qa_check_result_status_check CHECK (((status)::text = ANY ((ARRAY['pass'::character varying, 'warn'::character varying, 'fail'::character varying, 'skipped'::character varying, 'error'::character varying])::text[])))
);


--
-- Name: skill_qa_recording; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.skill_qa_recording (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    run_id uuid NOT NULL,
    skill_id uuid NOT NULL,
    content_hash text NOT NULL,
...
--
-- Name: skill_qa_run; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.skill_qa_run (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    skill_id uuid NOT NULL,
    version_number integer,
    content_hash text NOT NULL,
    inputs_fingerprint text NOT NULL,
    registry_version integer NOT NULL,
    tier character varying DEFAULT 'static'::character varying NOT NULL,
    trigger character varying DEFAULT 'manual'::character varying NOT NULL,
    status character varying NOT NULL,
    quality_score integer NOT NULL,
    quality_grade character varying NOT NULL,
    category_scores json NOT NULL,
    deductions json NOT NULL,
    blocking json DEFAULT '[]'::json NOT NULL,
    coverage json,
    enforcement_mode character varying DEFAULT 'advisory'::character varying NOT NULL,
    ...
    requested_by uuid,
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT skill_qa_run_enforcement_check CHECK (((enforcement_mode)::text = ANY ((ARRAY['advisory'::character varying, 'enforcing'::character varying])::text[]))),
    CONSTRAINT skill_qa_run_score_check CHECK (((quality_score >= 0) AND (quality_score <= 100))),
    CONSTRAINT skill_qa_run_status_check CHECK (((status)::text = ANY ((ARRAY['passed'::character varying, 'failed'::character varying, 'degraded'::character varying, 'skipped'::character varying])::text[]))),
    CONSTRAINT skill_qa_run_tier_check CHECK (((tier)::text = ANY ((ARRAY['static'::character varying, 'live'::character varying])::text[]))),
    CONSTRAINT skill_qa_run_trigger_check CHECK (((trigger)::text = ANY ((ARRAY['publish'::character varying, 'save'::character varying, 'approval'::character varying, 'manual'::character varying, 'import'::character varying])::text[])))
);


--
-- Name: skill_rating; Type: TABLE; Schema: public; Owner: -
--
...
--
-- Name: skill_scan; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.skill_scan (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    skill_id uuid NOT NULL,
    version_number integer,
    content_hash text NOT NULL,
    ...
    trust_grade character varying NOT NULL,
    deductions json NOT NULL,
    scan_source character varying DEFAULT 'manual'::character varying NOT NULL,
    scanned_by uuid,
    created_at timestamp without time zone DEFAULT CURRENT_TIMESTAMP NOT NULL,
    CONSTRAINT skill_scan_risk_level_check CHECK (((risk_level)::text = ANY ((ARRAY['low'::character varying, 'medium'::character varying, 'high'::character varying, 'critical'::character varying])::text[]))),
    CONSTRAINT skill_scan_source_check CHECK (((scan_source)::text = ANY ((ARRAY['import'::character varying, 'save'::character varying, 'approval'::character varying, 'manual'::character varying, 'scheduled'::character varying])::text[])))
);


--
-- Name: skill_submission; Type: TABLE; Schema: public; Owner: -
--

CREATE TABLE public.skill_submission (
    id uuid DEFAULT gen_random_uuid() NOT NULL,
    skill_id uuid NOT NULL,
    user_id uuid NOT NULL,
    status character varying DEFAULT 'pending'::character varying NOT NULL,
...
--
-- Name: assistant assistant_org_slug_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant
    ADD CONSTRAINT assistant_org_slug_unique UNIQUE (organization_id, slug);
...
--
-- Name: assistant_team assistant_team_assistant_id_team_id_pk; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant_team
    ADD CONSTRAINT assistant_team_assistant_id_team_id_pk PRIMARY KEY (assistant_id, team_id);
...
ALTER TABLE ONLY public.job_execution
    ADD CONSTRAINT job_execution_pkey PRIMARY KEY (idempotency_key);
...
--
-- Name: org_policy_version org_policy_version_number_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_policy_version
    ADD CONSTRAINT org_policy_version_number_unique UNIQUE (policy_id, version_number);


--
-- Name: org_policy_version org_policy_version_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_policy_version
    ADD CONSTRAINT org_policy_version_pkey PRIMARY KEY (id);
...
--
-- Name: organization_entitlement organization_entitlement_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.organization_entitlement
    ADD CONSTRAINT organization_entitlement_pkey PRIMARY KEY (organization_id);
...
--
-- Name: plugin_bundle_install plugin_bundle_install_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle_install
    ADD CONSTRAINT plugin_bundle_install_pkey PRIMARY KEY (id);


--
-- Name: plugin_bundle_install plugin_bundle_install_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle_install
    ADD CONSTRAINT plugin_bundle_install_unique UNIQUE (bundle_id, user_id);


--
-- Name: plugin_bundle_item plugin_bundle_item_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle_item
    ADD CONSTRAINT plugin_bundle_item_pkey PRIMARY KEY (id);


--
-- Name: plugin_bundle_item plugin_bundle_item_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle_item
    ADD CONSTRAINT plugin_bundle_item_unique UNIQUE (bundle_id, resource_type, resource_id);


--
-- Name: plugin_bundle plugin_bundle_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle
    ADD CONSTRAINT plugin_bundle_pkey PRIMARY KEY (id);


--
-- Name: plugin_bundle plugin_bundle_slug_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle
    ADD CONSTRAINT plugin_bundle_slug_unique UNIQUE (slug);


--
-- Name: plugin_source_entry plugin_source_entry_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_source_entry
    ADD CONSTRAINT plugin_source_entry_pkey PRIMARY KEY (id);


--
-- Name: plugin_source_entry plugin_source_entry_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_source_entry
    ADD CONSTRAINT plugin_source_entry_unique UNIQUE (source_id, name);


--
-- Name: plugin_source plugin_source_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_source
    ADD CONSTRAINT plugin_source_pkey PRIMARY KEY (id);


--
-- Name: plugin_source plugin_source_slug_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_source
    ADD CONSTRAINT plugin_source_slug_unique UNIQUE (slug);
...
--
-- Name: semantic_response_cache semantic_response_cache_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.semantic_response_cache
    ADD CONSTRAINT semantic_response_cache_pkey PRIMARY KEY (id);
...
--
-- Name: skill_attestation skill_attestation_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_attestation
    ADD CONSTRAINT skill_attestation_pkey PRIMARY KEY (id);


--
-- Name: skill_install skill_install_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_install
    ADD CONSTRAINT skill_install_pkey PRIMARY KEY (id);


--
-- Name: skill_install skill_install_skill_id_user_id_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_install
    ADD CONSTRAINT skill_install_skill_id_user_id_unique UNIQUE (skill_id, user_id);
...
--
-- Name: skill_qa_certification skill_qa_certification_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_certification
    ADD CONSTRAINT skill_qa_certification_pkey PRIMARY KEY (id);


--
-- Name: skill_qa_check_result skill_qa_check_result_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_check_result
    ADD CONSTRAINT skill_qa_check_result_pkey PRIMARY KEY (id);


--
-- Name: skill_qa_check_result skill_qa_check_result_run_check_uq; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_check_result
    ADD CONSTRAINT skill_qa_check_result_run_check_uq UNIQUE (run_id, check_id);


--
-- Name: skill_qa_recording skill_qa_recording_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_recording
    ADD CONSTRAINT skill_qa_recording_pkey PRIMARY KEY (id);


--
-- Name: skill_qa_recording skill_qa_recording_run_case_uq; Type: CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: skill_qa_run skill_qa_run_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_run
    ADD CONSTRAINT skill_qa_run_pkey PRIMARY KEY (id);


--
-- Name: skill_rating skill_rating_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_rating
    ADD CONSTRAINT skill_rating_pkey PRIMARY KEY (id);


--
-- Name: skill_rating skill_rating_skill_id_user_id_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_rating
    ADD CONSTRAINT skill_rating_skill_id_user_id_unique UNIQUE (skill_id, user_id);


--
-- Name: skill_scan skill_scan_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_scan
    ADD CONSTRAINT skill_scan_pkey PRIMARY KEY (id);


--
-- Name: skill_submission skill_submission_pkey; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_submission
    ADD CONSTRAINT skill_submission_pkey PRIMARY KEY (id);


--
-- Name: skill_version skill_version_number_unique; Type: CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_version
    ADD CONSTRAINT skill_version_number_unique UNIQUE (skill_id, version_number);
...
--
-- Name: assistant_agent_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX assistant_agent_id_idx ON public.assistant USING btree (agent_id);


--
-- Name: assistant_org_governance_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX assistant_org_governance_idx ON public.assistant USING btree (organization_id, governance_status);


--
-- Name: assistant_org_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX assistant_org_id_idx ON public.assistant USING btree (organization_id);


--
-- Name: assistant_published_slug_unique; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX assistant_published_slug_unique ON public.assistant USING btree (slug) WHERE (is_published AND is_active AND (deleted_at IS NULL));


--
-- Name: assistant_team_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX assistant_team_id_idx ON public.assistant USING btree (team_id);
...
--
-- Name: cron_run_log_one_running_per_job; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX cron_run_log_one_running_per_job ON public.cron_run_log USING btree (cron_job_id) WHERE (status = 'running'::text);
...
--
-- Name: event_outbox_dedup_key; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX event_outbox_dedup_key ON public.event_outbox USING btree (dedup_key);


--
-- Name: event_outbox_pending; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX event_outbox_pending ON public.event_outbox USING btree (created_at) WHERE (status = 'pending'::text);
...
--
-- Name: inference_request_log_assistant_id_created_at_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX inference_request_log_assistant_id_created_at_idx ON public.inference_request_log USING btree (assistant_id, created_at);
...
--
-- Name: invoice_org_external_id_uq; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX invoice_org_external_id_uq ON public.invoice USING btree (organization_id, external_invoice_id) WHERE (external_invoice_id IS NOT NULL);
...
--
-- Name: job_execution_running_claimed_at; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX job_execution_running_claimed_at ON public.job_execution USING btree (claimed_at) WHERE (status = 'running'::text);
...
--
-- Name: notification_user_active_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX notification_user_active_idx ON public.notification USING btree (user_id, created_at) WHERE (deleted_at IS NULL);
...
--
-- Name: org_invite_pending_email_uq; Type: INDEX; Schema: public; Owner: -
--

CREATE UNIQUE INDEX org_invite_pending_email_uq ON public.org_invite USING btree (organization_id, invited_email) WHERE (accepted_at IS NULL);
...
--
-- Name: org_policy_version_org_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX org_policy_version_org_idx ON public.org_policy_version USING btree (organization_id);


--
-- Name: org_policy_version_policy_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX org_policy_version_policy_idx ON public.org_policy_version USING btree (policy_id);
...
--
-- Name: org_role_permission_denied_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX org_role_permission_denied_idx ON public.org_role_permission USING btree (role_id) WHERE denied;
...
--
-- Name: plugin_bundle_install_user_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX plugin_bundle_install_user_idx ON public.plugin_bundle_install USING btree (user_id);


--
-- Name: plugin_bundle_item_bundle_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX plugin_bundle_item_bundle_idx ON public.plugin_bundle_item USING btree (bundle_id);


--
-- Name: plugin_bundle_org_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX plugin_bundle_org_idx ON public.plugin_bundle USING btree (organization_id);


--
-- Name: plugin_bundle_publisher_kind_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX plugin_bundle_publisher_kind_idx ON public.plugin_bundle USING btree (publisher_kind);


--
-- Name: plugin_bundle_source_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX plugin_bundle_source_idx ON public.plugin_bundle USING btree (source_id);


--
-- Name: plugin_bundle_status_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX plugin_bundle_status_idx ON public.plugin_bundle USING btree (status);


--
-- Name: plugin_source_entry_source_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX plugin_source_entry_source_idx ON public.plugin_source_entry USING btree (source_id);


--
-- Name: plugin_source_status_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX plugin_source_status_idx ON public.plugin_source USING btree (status);
...
--
-- Name: semantic_response_cache_e1024_hnsw_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX semantic_response_cache_e1024_hnsw_idx ON public.semantic_response_cache USING hnsw (embedding_1024 public.vector_cosine_ops);


--
-- Name: semantic_response_cache_e3072_hnsw_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX semantic_response_cache_e3072_hnsw_idx ON public.semantic_response_cache USING hnsw (embedding_3072 public.halfvec_cosine_ops);


--
-- Name: semantic_response_cache_e768_hnsw_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX semantic_response_cache_e768_hnsw_idx ON public.semantic_response_cache USING hnsw (embedding_768 public.vector_cosine_ops);


--
-- Name: semantic_response_cache_embedding_hnsw_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX semantic_response_cache_embedding_hnsw_idx ON public.semantic_response_cache USING hnsw (embedding public.vector_cosine_ops);


--
-- Name: semantic_response_cache_expires_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX semantic_response_cache_expires_idx ON public.semantic_response_cache USING btree (expires_at);


--
-- Name: semantic_response_cache_org_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX semantic_response_cache_org_idx ON public.semantic_response_cache USING btree (organization_id);


--
-- Name: semantic_response_cache_scope_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX semantic_response_cache_scope_idx ON public.semantic_response_cache USING btree (user_scope, model_id, static_fingerprint);


--
-- Name: skill_attestation_skill_created_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_attestation_skill_created_idx ON public.skill_attestation USING btree (skill_id, created_at DESC NULLS LAST);


--
-- Name: skill_attestation_skill_hash_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_attestation_skill_hash_idx ON public.skill_attestation USING btree (skill_id, content_hash);
...
--
-- Name: skill_install_user_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_install_user_id_idx ON public.skill_install USING btree (user_id);


--
-- Name: skill_is_published_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_is_published_idx ON public.skill USING btree (is_published);


--
-- Name: skill_org_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_org_idx ON public.skill USING btree (organization_id);


--
-- Name: skill_org_lifecycle_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_org_lifecycle_idx ON public.skill USING btree (organization_id, lifecycle_status);


--
-- Name: skill_qa_certification_skill_created_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_certification_skill_created_idx ON public.skill_qa_certification USING btree (skill_id, created_at DESC NULLS LAST);


--
-- Name: skill_qa_certification_skill_hash_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_certification_skill_hash_idx ON public.skill_qa_certification USING btree (skill_id, content_hash);


--
-- Name: skill_qa_check_result_run_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_check_result_run_idx ON public.skill_qa_check_result USING btree (run_id);


--
-- Name: skill_qa_check_result_skill_check_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_check_result_skill_check_idx ON public.skill_qa_check_result USING btree (skill_id, check_id, status);


--
-- Name: skill_qa_recording_skill_hash_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_recording_skill_hash_idx ON public.skill_qa_recording USING btree (skill_id, content_hash, created_at DESC NULLS LAST);


--
-- Name: skill_qa_run_certifying_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_run_certifying_idx ON public.skill_qa_run USING btree (skill_id, content_hash, registry_version, tier, created_at DESC NULLS LAST);


--
-- Name: skill_qa_run_fingerprint_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_run_fingerprint_idx ON public.skill_qa_run USING btree (inputs_fingerprint);


--
-- Name: skill_qa_run_skill_created_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_run_skill_created_idx ON public.skill_qa_run USING btree (skill_id, created_at DESC NULLS LAST);


--
-- Name: skill_qa_run_status_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_qa_run_status_idx ON public.skill_qa_run USING btree (status, created_at DESC NULLS LAST);


--
-- Name: skill_rating_skill_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_rating_skill_id_idx ON public.skill_rating USING btree (skill_id);


--
-- Name: skill_scan_skill_created_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_scan_skill_created_idx ON public.skill_scan USING btree (skill_id, created_at DESC NULLS LAST);


--
-- Name: skill_scan_skill_hash_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_scan_skill_hash_idx ON public.skill_scan USING btree (skill_id, content_hash);


--
-- Name: skill_scope_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_scope_idx ON public.skill USING btree (scope);


--
-- Name: skill_submission_skill_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_submission_skill_id_idx ON public.skill_submission USING btree (skill_id);


--
-- Name: skill_submission_status_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_submission_status_idx ON public.skill_submission USING btree (status);


--
-- Name: skill_submission_user_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_submission_user_id_idx ON public.skill_submission USING btree (user_id);
...
--
-- Name: skill_undeployed_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_undeployed_idx ON public.skill USING btree (user_id) WHERE (deployed = false);


--
-- Name: skill_user_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_user_id_idx ON public.skill USING btree (user_id);


--
-- Name: skill_user_id_visibility_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_user_id_visibility_idx ON public.skill USING btree (user_id, visibility);


--
-- Name: skill_version_skill_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX skill_version_skill_id_idx ON public.skill_version USING btree (skill_id);
...
--
-- Name: tool_usage_skill_id_idx; Type: INDEX; Schema: public; Owner: -
--

CREATE INDEX tool_usage_skill_id_idx ON public.tool_usage USING btree (skill_id);
...
--
-- Name: agent_deployment agent_deployment_owner_user_id_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.agent_deployment
    ADD CONSTRAINT agent_deployment_owner_user_id_user_id_fk FOREIGN KEY (owner_user_id) REFERENCES public."user"(id) ON DELETE CASCADE;
...
--
-- Name: assistant assistant_agent_id_agent_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant
    ADD CONSTRAINT assistant_agent_id_agent_id_fk FOREIGN KEY (agent_id) REFERENCES public.agent(id) ON DELETE SET NULL;


--
-- Name: assistant assistant_created_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant
    ADD CONSTRAINT assistant_created_by_user_id_fk FOREIGN KEY (created_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: assistant assistant_knowledge_base_id_knowledge_base_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant
    ADD CONSTRAINT assistant_knowledge_base_id_knowledge_base_id_fk FOREIGN KEY (knowledge_base_id) REFERENCES public.knowledge_base(id) ON DELETE SET NULL;


--
-- Name: assistant assistant_organization_id_organization_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant
    ADD CONSTRAINT assistant_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE CASCADE;


--
-- Name: assistant assistant_prompt_version_id_prompt_version_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant
    ADD CONSTRAINT assistant_prompt_version_id_prompt_version_id_fk FOREIGN KEY (prompt_version_id) REFERENCES public.prompt_version(id) ON DELETE SET NULL;


--
-- Name: assistant assistant_reviewed_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant
    ADD CONSTRAINT assistant_reviewed_by_user_id_fk FOREIGN KEY (reviewed_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: assistant_team assistant_team_assistant_id_assistant_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant_team
    ADD CONSTRAINT assistant_team_assistant_id_assistant_id_fk FOREIGN KEY (assistant_id) REFERENCES public.assistant(id) ON DELETE CASCADE;


--
-- Name: assistant assistant_team_id_team_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant
    ADD CONSTRAINT assistant_team_id_team_id_fk FOREIGN KEY (team_id) REFERENCES public.team(id) ON DELETE SET NULL;


--
-- Name: assistant_team assistant_team_team_id_team_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.assistant_team
    ADD CONSTRAINT assistant_team_team_id_team_id_fk FOREIGN KEY (team_id) REFERENCES public.team(id) ON DELETE CASCADE;
...
ALTER TABLE ONLY public.document_chunk
    ADD CONSTRAINT document_chunk_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE CASCADE;
...
--
-- Name: event_outbox event_outbox_organization_id_organization_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.event_outbox
    ADD CONSTRAINT event_outbox_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE CASCADE;
...
--
-- Name: inference_request_log inference_request_log_assistant_id_assistant_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.inference_request_log
    ADD CONSTRAINT inference_request_log_assistant_id_assistant_id_fk FOREIGN KEY (assistant_id) REFERENCES public.assistant(id) ON DELETE SET NULL;
...
--
-- Name: job_execution job_execution_organization_id_organization_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.job_execution
    ADD CONSTRAINT job_execution_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE CASCADE;
...
--
-- Name: org_policy_version org_policy_version_changed_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_policy_version
    ADD CONSTRAINT org_policy_version_changed_by_user_id_fk FOREIGN KEY (changed_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: org_policy_version org_policy_version_organization_id_organization_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.org_policy_version
    ADD CONSTRAINT org_policy_version_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE CASCADE;
...
--
-- Name: organization_entitlement organization_entitlement_organization_id_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--
...
--
-- Name: organization_entitlement organization_entitlement_updated_by_fkey; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.organization_entitlement
    ADD CONSTRAINT organization_entitlement_updated_by_fkey FOREIGN KEY (updated_by) REFERENCES public."user"(id) ON DELETE SET NULL;
...
--
-- Name: plugin_bundle_install plugin_bundle_install_bundle_id_plugin_bundle_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle_install
    ADD CONSTRAINT plugin_bundle_install_bundle_id_plugin_bundle_id_fk FOREIGN KEY (bundle_id) REFERENCES public.plugin_bundle(id) ON DELETE CASCADE;


--
-- Name: plugin_bundle_install plugin_bundle_install_organization_id_organization_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle_install
    ADD CONSTRAINT plugin_bundle_install_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE SET NULL;


--
-- Name: plugin_bundle_install plugin_bundle_install_user_id_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle_install
    ADD CONSTRAINT plugin_bundle_install_user_id_user_id_fk FOREIGN KEY (user_id) REFERENCES public."user"(id) ON DELETE CASCADE;


--
-- Name: plugin_bundle_item plugin_bundle_item_bundle_id_plugin_bundle_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle_item
    ADD CONSTRAINT plugin_bundle_item_bundle_id_plugin_bundle_id_fk FOREIGN KEY (bundle_id) REFERENCES public.plugin_bundle(id) ON DELETE CASCADE;


--
-- Name: plugin_bundle plugin_bundle_organization_id_organization_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle
    ADD CONSTRAINT plugin_bundle_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE SET NULL;


--
-- Name: plugin_bundle plugin_bundle_reviewed_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle
    ADD CONSTRAINT plugin_bundle_reviewed_by_user_id_fk FOREIGN KEY (reviewed_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: plugin_bundle plugin_bundle_source_id_plugin_source_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle
    ADD CONSTRAINT plugin_bundle_source_id_plugin_source_id_fk FOREIGN KEY (source_id) REFERENCES public.plugin_source(id) ON DELETE SET NULL;


--
-- Name: plugin_bundle plugin_bundle_submitted_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle
    ADD CONSTRAINT plugin_bundle_submitted_by_user_id_fk FOREIGN KEY (submitted_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: plugin_bundle plugin_bundle_team_id_team_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_bundle
    ADD CONSTRAINT plugin_bundle_team_id_team_id_fk FOREIGN KEY (team_id) REFERENCES public.team(id) ON DELETE SET NULL;


--
-- Name: plugin_source plugin_source_added_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_source
    ADD CONSTRAINT plugin_source_added_by_user_id_fk FOREIGN KEY (added_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: plugin_source_entry plugin_source_entry_source_id_plugin_source_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.plugin_source_entry
    ADD CONSTRAINT plugin_source_entry_source_id_plugin_source_id_fk FOREIGN KEY (source_id) REFERENCES public.plugin_source(id) ON DELETE CASCADE;
...
--
-- Name: semantic_response_cache semantic_response_cache_organization_id_organization_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.semantic_response_cache
    ADD CONSTRAINT semantic_response_cache_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE CASCADE;
...
--
-- Name: skill_attestation skill_attestation_revoked_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_attestation
    ADD CONSTRAINT skill_attestation_revoked_by_user_id_fk FOREIGN KEY (revoked_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: skill_attestation skill_attestation_signed_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_attestation
    ADD CONSTRAINT skill_attestation_signed_by_user_id_fk FOREIGN KEY (signed_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: skill_attestation skill_attestation_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_attestation
    ADD CONSTRAINT skill_attestation_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_install skill_install_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_install
    ADD CONSTRAINT skill_install_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_install skill_install_user_id_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_install
    ADD CONSTRAINT skill_install_user_id_user_id_fk FOREIGN KEY (user_id) REFERENCES public."user"(id) ON DELETE CASCADE;


--
-- Name: skill skill_lifecycle_reviewed_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill
    ADD CONSTRAINT skill_lifecycle_reviewed_by_user_id_fk FOREIGN KEY (lifecycle_reviewed_by) REFERENCES public."user"(id) ON DELETE SET NULL;
...
ALTER TABLE ONLY public.skill
    ADD CONSTRAINT skill_organization_id_organization_id_fk FOREIGN KEY (organization_id) REFERENCES public.organization(id) ON DELETE SET NULL;


--
-- Name: skill_qa_certification skill_qa_certification_issued_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_certification
    ADD CONSTRAINT skill_qa_certification_issued_by_user_id_fk FOREIGN KEY (issued_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: skill_qa_certification skill_qa_certification_revoked_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_certification
    ADD CONSTRAINT skill_qa_certification_revoked_by_user_id_fk FOREIGN KEY (revoked_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: skill_qa_certification skill_qa_certification_run_id_skill_qa_run_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_certification
    ADD CONSTRAINT skill_qa_certification_run_id_skill_qa_run_id_fk FOREIGN KEY (run_id) REFERENCES public.skill_qa_run(id) ON DELETE SET NULL;


--
-- Name: skill_qa_certification skill_qa_certification_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_certification
    ADD CONSTRAINT skill_qa_certification_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_qa_check_result skill_qa_check_result_run_id_skill_qa_run_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_check_result
    ADD CONSTRAINT skill_qa_check_result_run_id_skill_qa_run_id_fk FOREIGN KEY (run_id) REFERENCES public.skill_qa_run(id) ON DELETE CASCADE;


--
-- Name: skill_qa_check_result skill_qa_check_result_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_check_result
    ADD CONSTRAINT skill_qa_check_result_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_qa_recording skill_qa_recording_run_id_skill_qa_run_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_recording
    ADD CONSTRAINT skill_qa_recording_run_id_skill_qa_run_id_fk FOREIGN KEY (run_id) REFERENCES public.skill_qa_run(id) ON DELETE CASCADE;


--
-- Name: skill_qa_recording skill_qa_recording_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_recording
    ADD CONSTRAINT skill_qa_recording_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_qa_run skill_qa_run_requested_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_run
    ADD CONSTRAINT skill_qa_run_requested_by_user_id_fk FOREIGN KEY (requested_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: skill_qa_run skill_qa_run_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_qa_run
    ADD CONSTRAINT skill_qa_run_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_rating skill_rating_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_rating
    ADD CONSTRAINT skill_rating_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_rating skill_rating_user_id_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_rating
    ADD CONSTRAINT skill_rating_user_id_user_id_fk FOREIGN KEY (user_id) REFERENCES public."user"(id) ON DELETE CASCADE;


--
-- Name: skill_scan skill_scan_scanned_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_scan
    ADD CONSTRAINT skill_scan_scanned_by_user_id_fk FOREIGN KEY (scanned_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: skill_scan skill_scan_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_scan
    ADD CONSTRAINT skill_scan_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_submission skill_submission_reviewed_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_submission
    ADD CONSTRAINT skill_submission_reviewed_by_user_id_fk FOREIGN KEY (reviewed_by) REFERENCES public."user"(id);


--
-- Name: skill_submission skill_submission_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_submission
    ADD CONSTRAINT skill_submission_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;


--
-- Name: skill_submission skill_submission_user_id_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_submission
    ADD CONSTRAINT skill_submission_user_id_user_id_fk FOREIGN KEY (user_id) REFERENCES public."user"(id) ON DELETE CASCADE;
...
--
-- Name: skill skill_user_id_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill
    ADD CONSTRAINT skill_user_id_user_id_fk FOREIGN KEY (user_id) REFERENCES public."user"(id) ON DELETE CASCADE;


--
-- Name: skill_version skill_version_created_by_user_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_version
    ADD CONSTRAINT skill_version_created_by_user_id_fk FOREIGN KEY (created_by) REFERENCES public."user"(id) ON DELETE SET NULL;


--
-- Name: skill_version skill_version_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.skill_version
    ADD CONSTRAINT skill_version_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE CASCADE;
...
--
-- Name: tool_usage tool_usage_skill_id_skill_id_fk; Type: FK CONSTRAINT; Schema: public; Owner: -
--

ALTER TABLE ONLY public.tool_usage
    ADD CONSTRAINT tool_usage_skill_id_skill_id_fk FOREIGN KEY (skill_id) REFERENCES public.skill(id) ON DELETE SET NULL;
...
\unrestrict Uu3m5rL41VgHitXbh0cfXyNepzrPH6jlKIYbFespjxsrCRmtAraGEkkPrgfAn9M

--
-- HAND-PATCHED ADDITIONS (2026-08-11, carried forward 2026-08-25) — objects the
-- drizzle-derived export baseline cannot express. Kept at the end, after the
-- dump's \unrestrict, so regenerated dumps diff cleanly above this marker.
--
-- (skill_undeployed_idx, patched in here since 1.9.0, is now declared in
-- schema.pg.ts and comes out of the dump itself — see the CREATE INDEX above.)
--
-- ADR-0037 (app migration 0071): admin_audit_log is append-only AT THE
-- DATABASE. drizzle-kit push cannot express functions/triggers, so no
-- packaged schema before 1.9.0 carried the control; delta-migrated databases
-- get it from migrate-1.9.0.sql — this keeps fresh installs identical.
-- Verbatim from 0071_governance_engine.sql (function name schema-qualified:
-- the dump above pins search_path='' for the rest of the session):
...
END;
$$;
DROP TRIGGER IF EXISTS admin_audit_log_immutable_trg ON public.admin_audit_log;
