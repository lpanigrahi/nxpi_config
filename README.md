...
`redis_cache_url` **file secret**. That file is generated by `./install.sh`, and
**only** by `./install.sh`. So on an existing deployment adopting this version:
...
lands at *container create* —

```
invalid mount config for type "bind": bind source path does not exist: …/secrets/redis_cache_url
```

— which in `update.sh` is step 6, **after** the safety backup, the image pull
and the schema sync have already run. The automatic rollback then re-runs the
same `compose up` and hits the same bind failure, so it cannot recover either
and the app stays down until you run `./install.sh`. The schema sync is
